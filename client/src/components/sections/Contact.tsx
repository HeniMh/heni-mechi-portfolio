...
export function Contact() {
  ...
  return (
    <section id="contact" className="fixed bottom-5 right-5 z-[90] w-[min(430px,calc(100%-32px))] md:bottom-6 md:right-6">
      {!open && (
        <button type="button" onClick={() => setOpen(true)} className="group ml-auto flex items-center gap-3 rounded-full border border-black/10 bg-[#d9ff63] px-5 py-3.5 text-sm font-extrabold text-black shadow-[0_18px_45px_rgba(0,0,0,.2)] transition hover:-translate-y-1 hover:shadow-[0_22px_55px_rgba(0,0,0,.28)] dark:border-white/10" aria-expanded="false" aria-controls="contact-panel">
      ...
      {open && <div id="contact-panel" className="animate-[contactPanelIn_.25s_ease-out_both] overflow-hidden rounded-[2rem] border border-white/15 bg-[#111318]/95 text-white shadow-[0_24px_80px_rgba(0,0,0,.4)] backdrop-blur-2xl">
        ...
        <div className="relative bg-[#111318] px-5 pb-5 pt-6 text-white">
          ...
          <h2 className="relative max-w-md font-display text-2xl font-extrabold leading-[1.02] tracking-[-.03em]">Have a product, role or automation challenge?</h2>
          <p className="relative mt-3 max-w-md text-xs font-medium leading-6 text-white/50">Available for front-end / full-stack opportunities, product missions, modernization work and AI-enabled automation projects.</p>
        ...
        <form onSubmit={onSubmit} noValidate className="grid gap-3 border-t border-white/10 bg-[#f5f2ea] p-5 text-[#111318] dark:bg-[#111318] dark:text-white" aria-label="Contact form">
          <div className="mb-1">
            <span className="text-[10px] font-extrabold uppercase tracking-[.16em] text-black/45 dark:text-white/35">Send a message</span>
            <p className="mt-1 text-xs font-medium text-black/50 dark:text-white/45">Tell me what you are building and where you need help.</p>
          </div>
